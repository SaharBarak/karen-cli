...
exports.detectDesignSystemChaos = detectDesignSystemChaos;
const id_js_1 = require("../utils/id.js");
/**
 * Main design system chaos detection function
 */
function detectDesignSystemChaos(snapshots) {
    const issues = [];
 ...
 */
function analyzeButtonVariations(snapshots) {
    const issues = [];
    const buttons = [];
    // Collect all buttons across viewports
    for (const snapshot of snapshots) {
        const buttonElements = snapshot.dom.filter((el) => el.tagName === 'BUTTON' ||
            el.tagName === 'A' && (el.classes.some((c) => c.includes('btn') || c.includes('button')) ||
 ...
 */
function analyzeInputVariations(snapshots) {
    const issues = [];
    const inputs = [];
    // Collect all input fields across viewports
    for (const snapshot of snapshots) {
        const inputElements = snapshot.dom.filter((el) => el.tagName === 'INPUT' ||
            el.tagName === 'TEXTAREA' ||
            el.tagName === 'SELECT' ||
            el.classes.some((c) => c.includes('input') || c.includes('field')));
 ...
 */
function analyzeCardVariations(snapshots) {
    const issues = [];
    const cards = [];
    // Collect all card-like elements across viewports
    for (const snapshot of snapshots) {
        const cardElements = snapshot.dom.filter((el) => el.classes.some((c) => c.includes('card') || c.includes('panel') || c.includes('box')) ||
 ...
 */
function analyzeHeadingInconsistencies(snapshots) {
    const issues = [];
    const headings = new Map();
    // Collect all headings across viewports
    for (const snapshot of snapshots) {
        const headingElements = snapshot.dom.filter((el) => ['H1', 'H2', 'H3', 'H4', 'H5', 'H6'].includes(el.tagName));
