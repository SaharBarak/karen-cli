 ...
 */
function detectMisalignment(snapshots, config) {
    const issues = [];
 ...
 */
function checkHorizontalAlignment(snapshot, tolerance) {
    const issues = [];
    ...
    for (const row of rows) {
        ...
        if (misalignment > tolerance) {
            ...
            const baseline = Array.from(positionCounts.entries()).reduce((a, b) => a[1] > b[1] ? a : b)[0];
            // Find misaligned elements
            const misalignedElements = row.filter((el) => Math.abs(el.rect.y - baseline) > tolerance);
            issues.push({
                id: (0, id_js_1.generateId)('ALN'),
                type: 'design-system',
                severity: 'medium',
                ...
                fix: {
                    ...
                    code: {
                        ...
                        explanation: 'Using flexbox ensures all items in a row maintain consistent vertical alignment',
                    },
                },
            });
        }
 ...
 */
function checkVerticalAlignment(snapshot, tolerance) {
    const issues = [];
    ...
    for (const column of columns) {
        ...
        if (misalignment > tolerance) {
            ...
            const baseline = Array.from(positionCounts.entries()).reduce((a, b) => a[1] > b[1] ? a : b)[0];
            const misalignedElements = column.filter((el) => Math.abs(el.rect.x - baseline) > tolerance);
            issues.push({
                id: (0, id_js_1.generateId)('ALN'),
                type: 'design-system',
                severity: 'medium',
                ...
                fix: {
                    ...
                    code: {
                        ...
                        explanation: 'Using CSS Grid ensures consistent column alignment and spacing',
                    },
                },
            });
        }
 ...
 */
function checkGridAlignment(snapshot, _tolerance) {
    const issues = [];
    // Find potential grid containers (elements with multiple children)
    const gridContainers = snapshot.dom.filter((el) => el.children && el.children.length >= 4);
    for (const container of gridContainers) {
        ...
        if (gaps.horizontal.size > 3 || gaps.vertical.size > 3) {
            ...
            const avgHorizontalGap = Array.from(gaps.horizontal.values()).reduce((a, b) => a + b, 0) /
                gaps.horizontal.size;
            const avgVerticalGap = Array.from(gaps.vertical.values()).reduce((a, b) => a + b, 0) /
                gaps.vertical.size;
            issues.push({
                id: (0, id_js_1.generateId)('ALN'),
                type: 'design-system',
 ...
 */
function groupElementsByRow(elements, tolerance) {
 ...
 */
function groupElementsByColumn(elements, tolerance) {
 ...
 */
function calculateGaps(elements) {
    const horizontalGaps = new Map();
    const verticalGaps = new Map();
    // Sort by position
