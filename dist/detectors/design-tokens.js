...
exports.enforceDesignTokens = enforceDesignTokens;
const id_js_1 = require("../utils/id.js");
/**
 * Main design token enforcement function
 */
function enforceDesignTokens(snapshots, config) {
    const issues = [];
 ...
 */
function checkSpacingTokens(element, snapshot, config) {
    const issues = [];
    // Check padding
    ...
    if (padding && !padding.includes('var(') && padding !== '0px') {
        ...
        const allOnScale = paddingValues.every((val) => config.spacingScale.includes(val));
        if (allOnScale && paddingValues.some((v) => v > 0)) {
            const tokenNames = paddingValues.map((val) => {
                const index = config.spacingScale.indexOf(val);
                return `var(--spacing-${index})`;
            });
            issues.push({
                id: (0, id_js_1.generateId)('DT'),
                type: 'design-system',
                severity: 'low',
    ...
    if (margin && !margin.includes('var(') && margin !== '0px' && margin !== 'auto') {
        ...
        const allOnScale = marginValues.every((val) => val === 0 || config.spacingScale.includes(val));
        if (allOnScale && marginValues.some((v) => v > 0)) {
            const tokenNames = marginValues.map((val) => {
                if (val === 0)
                    return '0';
                const index = config.spacingScale.indexOf(val);
                return `var(--spacing-${index})`;
 ...
 */
function checkColorTokens(element, snapshot, config) {
    const issues = [];
 ...
 */
function checkBorderRadiusTokens(element, snapshot) {
    const issues = [];
 ...
 */
function checkTypographyTokens(element, snapshot, config) {
    const issues = [];
