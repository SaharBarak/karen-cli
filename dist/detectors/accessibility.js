...
function generateAccessibleColor(bg, _requiredRatio) {
...
function checkAccessibility(snapshots, config) {
    const issues = [];
    for (const snapshot of snapshots) {
        const textElements = snapshot.dom.filter((el) => el.computedStyle.fontSize && parseFloat(el.computedStyle.fontSize) > 0);
        for (const element of textElements) {
            ...
            const requiredRatio = fontSize < 18 ? config.contrastRatios.AAA : config.contrastRatios.AA;
