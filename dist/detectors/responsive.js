...
exports.enforceResponsiveDesign = enforceResponsiveDesign;
const id_js_1 = require("../utils/id.js");
/**
 * Main responsive design enforcement function
 */
function enforceResponsiveDesign(snapshots, config) {
    const issues = [];
 ...
 */
function detectNonResponsiveElements(snapshots, _config) {
    const issues = [];
    ...
    for (const [selector, viewportData] of elementsBySelector) {
        if (viewportData.size < 3)
            continue; // Need at least 3 viewports to compare
        const viewports = Array.from(viewportData.entries());
        const fontSizes = viewports.map(([, data]) => parseFloat(data.element.computedStyle.fontSize || '16'));
                ...
                issues.push({
                    ...
                    fix: {
                        ...
                        code: {
                            ...
                            explanation: 'clamp() allows text to scale fluidly between minimum (mobile) and maximum (desktop) sizes',
                        },
                    },
                });
            }
        ...
        const fixedWidthCount = widths.filter((w) => w && w.endsWith('px') && parseInt(w) > 600).length;
 ...
 */
function detectFixedWidths(snapshots) {
    const issues = [];
    // Check on mobile viewport only
    const mobileSnapshot = snapshots.find((s) => s.viewport.width <= 414 && s.viewport.width >= 320);
    if (!mobileSnapshot)
        return issues;
 ...
 */
function detectResponsiveOverflow(snapshots) {
    const issues = [];
    ...
    if (!desktopSnapshot || !mobileSnapshot)
        return issues;
    ...
    for (const desktopElement of desktopSnapshot.dom) {
        if (!mobileSelectors.has(desktopElement.selector))
            continue;
        const mobileElement = mobileSnapshot.dom.find((el) => el.selector === desktopElement.selector);
        if (!mobileElement)
            continue;
