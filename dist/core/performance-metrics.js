...
exports.collectWebVitals = collectWebVitals;
 ...
 */
async function analyzeFonts(page) {
    return page.evaluate(() => {
        let issues = 0;
        ...
        const fontResources = performance
            ...
            .filter((r) => r.initiatorType === 'css' && /\.(woff2?|ttf|otf|eot)$/i.test(r.name));
...
function calculatePerformanceScore(metrics) {
    ...
    if (interactionMetric !== undefined) {
        weights += 25;
        if (interactionMetric > 500)
            score -= 25; // Poor
        else if (interactionMetric > 200)
            score -= 15; // Needs improvement
        else
            score -= 0; // Good
    }
